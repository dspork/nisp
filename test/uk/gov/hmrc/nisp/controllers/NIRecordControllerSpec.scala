...
import uk.gov.hmrc.nisp.helpers.{TestAccountBuilder, StubNIService}
...
class NIRecordControllerSpec extends UnitSpec with OneAppPerSuite {
  ...
  def testNIRecordController(newAPIConfig: Boolean = false): NIRecordController = new NIRecordController {
    override def niService: NIResponseService = StubNIService
  }
  ...
  "return 404 if incorrect NINO" in {
    val result = testNIRecordController().getNIRecord(nonExistentNino)(FakeRequest())
