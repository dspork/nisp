...
class SPContextMessageSpec extends UnitSpec {
  ...
  "serialised into JSON" should {
    "output ScenarioOne when ScenarioOne is formatted" in {
      Json.toJson(SPContextMessage.ScenarioOne) shouldBe JsString("ScenarioOne")
    }

    "parse ScenarioOne when ScenarioOne is read" in {
      Json.fromJson[SPContextMessage](JsString("ScenarioOne")).get shouldBe SPContextMessage.ScenarioOne
    }
  }
