...
import uk.gov.hmrc.nisp.helpers.{StubNpsConnector, StubSPResponseService, TestAccountBuilder}
...
class SPResponseServiceSpec extends UnitSpec with MockitoSugar with OneAppPerSuite {
  ...
  "return failed Future for a non-existent NINO" in {
    val spResponse = StubSPResponseService.getSPResponse(nonexistentnino)
  ...
  "age Calculation" should {
    "return 30 age for date of Birth 10/05/1980 and current date 10/7/2010" in {
      new StubSPResponseService(localDate = new LocalDate(2010, 7, 10)).getAge(NpsDate(1980, 5, 10)) shouldBe 30
    }

    "return 9 age for date of Birth 10/05/2001 and current date 10/7/2010" in {
      new StubSPResponseService(localDate = new LocalDate(2010, 7, 10)).getAge(NpsDate(2001, 5, 10)) shouldBe 9
    }

    "return 59 age for date of Birth 19/07/1950 and current date 10/7/2010" in {
      new StubSPResponseService(localDate = new LocalDate(2010, 7, 10)).getAge(NpsDate(1950, 7, 19)) shouldBe 59
    }

    "return 50 age for date of Birth 10/07/1950 and current date 10/7/2000" in {
      new StubSPResponseService(localDate = new LocalDate(2000, 7, 10)).getAge(NpsDate(1950, 7, 10)) shouldBe 50
    }

    "return 49 age for date of Birth 10/07/1950 and current date 11/7/1999" in {
      new StubSPResponseService(localDate = new LocalDate(1999, 7, 11)).getAge(NpsDate(1950, 7, 10)) shouldBe 49
    }

    "return 48 age for date of Birth 10/07/1950 and current date 9/7/1999" in {
      new StubSPResponseService(localDate = new LocalDate(1999, 7, 9)).getAge(NpsDate(1950, 7, 10)) shouldBe 48
    }
  }
