...
trait CitizenDetailsService {
  ...
  def retrieveMCIStatus(nino: Nino)(implicit hc: HeaderCarrier): Future[Boolean] = {
    citizenDetailsConnector.connectToGetPersonDetails(nino).map(status => status == LOCKED)
  }
